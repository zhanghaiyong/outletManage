...
+(instancetype)lockOnView:(UIView*)view offsetFromBottom:(CGFloat)offset onPasswordSet:(void (^)(DBGuestureLock *lock, NSString *password))onPasswordSet onGetCorrectPswd:(void (^)(DBGuestureLock *lock, NSString *password))onGetCorrectPswd onGetIncorrectPswd:(void (^)(DBGuestureLock *lock, NSString *password))onGetIncorrectPswd {
    CGFloat span   = ([UIScreen mainScreen].bounds.size.width-buttonSize*3)/4;
    CGFloat width  = [UIScreen mainScreen].bounds.size.width;
    CGFloat height = buttonSize*3+span*2;
    CGRect frame   = CGRectMake(0, 150, width, height);
    
    NSLog(@"%f  %f",[UIScreen mainScreen].bounds.size.width,span);
...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) { // Draw 9 Lock Buttons
        ...
        CGFloat spacing = ([UIScreen mainScreen].bounds.size.width-buttonSize*3)/4; //Split into 10 Part
        NSLog(@"%f",spacing);
        CGFloat radius = buttonSize;
...
//            NSInteger col = i%3; //3 buttons for each column
            ...
            CGRect frame = CGRectMake(spacing+i/3*(radius+spacing), 0+i%3*(radius+spacing), radius, radius);
...
-(void)setPropertiesByState:(DBButtonState)buttonState {
    switch (buttonState) {
        case DBButtonStateNormal:
            ...
            self.strokeWidth = 2.f; //As default
            if (self.delegate) {
            ...
            //As default
            self.strokeColor = [Uitils colorWithHex:0xBFC6CE];
            if (self.delegate) {
        ...
        case DBButtonStateSelected:
            ...
            self.centerPointRadius = 12.f; //As default
            if (self.delegate){
                if ([self.delegate respondsToSelector:@selector(radiusOfButtonCircleCenterPointOnState:)]) {
            ...
            self.lineColor = [Uitils colorWithHex:buttonTitleColor];
            if (self.delegate) {
                if ([self.delegate respondsToSelector:@selector(lineColorOfGuestureOnState:)]) {
            ...
            //As default
            self.centerPointColor = [Uitils colorWithHex:buttonTitleColor];
            if (self.delegate) {
                if ([self.delegate respondsToSelector:@selector(colorOfButtonCircleCenterPointOnState:)]) {
